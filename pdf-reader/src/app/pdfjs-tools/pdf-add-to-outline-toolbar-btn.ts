import { PdfAddToOutlineEditor } from './pdf-add-to-outline-editor';
import { PdfToolbarBtn } from './pdf-toolbar-btn';

export class PdfAddToOutlineToolbarBtn extends PdfToolbarBtn {

  private _getAddToOutline(): PdfAddToOutlineEditor { return this.registry.get('add-to-outline-editor'); }
  ...
  protected override getClassName() { return 'add-to-outline'; }
  protected override getTitle() { return 'Add to Outline'; }

  protected override selected() {
    this._getAddToOutline().setEnabled(true);
    this._getToolbarEl().showDetails(null as any);
  }

  protected override unselected() {
    this._getAddToOutline().setEnabled(false);
    this._getToolbarEl().showDetails(null as any);
  }
}