...
export class PdfFreeformToolbarBtn extends PdfToolbarBtn {
  ...
  protected override getTitle() { return 'Pen / Draw'; }
  ...
  protected override selected() {
    ...
    this._getToolbarEl().setActiveMode('freeform');
  ...
  private getToolbarDetailsEl() {
    const wrapper = htmlToElements(
      `<div class="pdf-annotation-panel">
        <div class="pdf-annotation-panel__header">Pen / Draw</div>
        <div class="pdf-annotation-panel__hint">Draw on the page, then click Done to save your stroke.</div>
      </div>`);

    wrapper.appendChild(this._getColorsEl());
    wrapper.appendChild(this._getStrokesEl());
    wrapper.appendChild(this._getDoneEl());
    return [wrapper];
  }
  ...
  private _getDoneEl() {
    const el = htmlToElements(
      `<div class="pdf-annotation-panel__actions">
        <button type="button" class="pdf-annotation-btn pdf-annotation-btn--primary">Done</button>
      </div>`);
  ...
  private _getStrokesEl() {
    ...
    const storkesEl = htmlToElements(
      `<div class="pdf-annotation-panel__section">
        <div class="pdf-annotation-panel__section-label">Stroke Width</div>
        <div class="pdf-annotation-range ${className}">
          ...
          <span class="pdf-annotation-range__value pdf-annotation-toolbar__stroke-value">${config.value}px</span>
        </div>
       </div>`);

    const strokeInputEl = storkesEl.querySelector('input') as HTMLInputElement;
    const strokeSpanEl = storkesEl.querySelector('.pdf-annotation-toolbar__stroke-value') as HTMLElement;
