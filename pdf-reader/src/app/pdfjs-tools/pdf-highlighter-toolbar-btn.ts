...
export class PdfHighlighterToolbarBtn extends PdfToolbarBtn {
  ...
  private get colorDotEl() { return this.button.querySelector(`.color-dot`) as HTMLElement; }
  protected getColorOptions() { return ['#ffd40075', '#ff656375', '#5db22175', '#2ba8e875', '#a28ae975', '#e66df275', '#f2982375', '#aaaaaa75', 'black']; }
  ...
  protected override selected() {
    ...
    this._setType(this.getType().type);
  ...
  protected override _addToolbarUI() {
    ...
    this.button.addEventListener('mouseover', () => {
      if (this._isEnabled() && this._getType() == this.getType().type && !this._getToolbarEl().hasDetails()) {
        this._getToolbarEl().showDetails(this.getToolbarDetailsEl());
      }
    });

    this._getDocument().addEventListener('mousedown', ($event: any) => {
      if (this._isEnabled() && this._getType() == this.getType().type && isLeftClick($event)) {
