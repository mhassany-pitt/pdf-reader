            ...
            const rectEl = htmlToElements(
              `<div 
                ...
                title="${annotTitleAttr(annot, annotIsMine(annot)
                  ? (annot.note?.length ? 'Click to edit note' : 'Click to add a note')
                ...
                class="
                  ...
                  ${!annot.note?.length && i == 0 ? 'pdf-annotation--noteable' : ''}
      ...
      .appendChild(htmlToElements(
        `<style>
          ...
          .pdf-annotation__rect:hover {
            box-shadow: 0 0 0 2px rgba(61, 109, 240, 0.28);
          }

          .pdf-annotation--has-note::after,
          .pdf-annotation--noteable:hover::after {
            content: "";
            ...
            position: absolute;
            left: calc(var(--annotation-scale) * -1 * var(--note-icon-dimension) * 0.35);
            top: calc(var(--annotation-scale) * -1 * var(--note-icon-dimension) * 0.35);
