...
export class HelperAnnotator {
  ...
  private _registerAnnotatorItemUI() {
      ...
      const radio = (elSelector, setValue) => {
        ...
        parent.addEventListener('click', ($event: any) => {
          ...
          if (el.tagName.toLowerCase() == 'button') {
            parent.querySelectorAll('button').forEach((button: any) => button.classList.remove('helper-annotation__btn--selected'));
            el.classList.add('helper-annotation__btn--selected');
...
(window as any).helper_annotator = function init({ iframe, pdfjs, storage, annotator }) {
